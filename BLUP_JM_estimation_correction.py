...
data_E = np.delete(data, 5, axis=1)
itmax = 1000
epsilon = 0.005
sigma_err = 0.5
n = len(data_E)
...
# Monte Carlo
# ------------------------------------------------------------------------------
r = np.column_stack((data_E, z1))
idex = np.argsort((data_E[:,2]))
r = r[idex,:]
X_E = r[:,4:num+4]
T = r[:,2:3]
delta = r[:,3:4]
...
for k in range(K):
    TMB_C[:, k] = (X_E[:,1:2] + np.random.normal(0,
                   sigma_err, (n, 1))*1j).reshape(-1)
#-----------------------------------------------------------------------------
def Score_C(betaR1,betaR2,betaT,u1,u2,v,theta1,theta2,theta3,ro1,ro2,ro3,TMB_C,X_E,z,deltaT,R1,R2,M):  
    sc1 = 0
    sc2 = 0
    sc3 = 0
    sc4 = 0
    for i in range(K):
        X_C = np.column_stack((X_E[:,0:1],TMB_C[:, i:i+1]))
        yetaR1_C = np.dot(X_C, betaR1) + np.dot(z,u1)
        yetaR2_C = np.dot(X_C, betaR2) + np.dot(z,u2)
        FirstyetaR1_C = R1 - np.exp(yetaR1_C) / (1 + np.exp(yetaR1_C) + np.exp(yetaR2_C))
        FirstyetaR2_C = R2 - np.exp(yetaR2_C) / (1 + np.exp(yetaR1_C) + np.exp(yetaR2_C))       
        sc1 += (np.dot(X_C.T, FirstyetaR1_C)).real
        sc2 += (np.dot(X_C.T, FirstyetaR2_C)).real
        sc3 += np.dot(z.T, FirstyetaR1_C.real)
        sc4 += np.dot(z.T, FirstyetaR2_C.real)
        
    FirstbetaR1_C = sc1/K
    FirstbetaR2_C = sc2/K
        
    yetaT = X_E[:,0:1]* betaT[0] + X_E[:,1:2]*betaT[1] + np.dot(z,v)   
    A = deltaT / np.dot(M.T, (np.exp(yetaT)))  
    FirstyetaT = deltaT - np.exp(yetaT) * np.dot(M, A)
    FirstbetaT2_1 = np.sum(deltaT*(X_E[:,0:1] - np.dot(M.T, X_E[:,0:1]*np.exp(yetaT))
                                                     /np.dot(M.T, np.exp(yetaT))))
    FirstbetaT2_2 = np.sum(deltaT*(X_E[:,1:2] - np.dot(M.T, X_E[:,1:2]*np.exp(yetaT))
                                                     /np.dot(M.T, np.exp(yetaT)) 
                                              + (sigma_err**2)*betaT[1]))
   
    FirstbetaT_C = np.array((FirstbetaT2_1,FirstbetaT2_2)).reshape(-1,1)
    
    Firstu1_C = sc3/K-(( u1*(1-ro3**2) * theta2 * theta3
    ...
    Firstu2_C = sc4/K-((-u1*(ro1-ro2*ro3) * theta3 * math.sqrt(theta1 * theta2)
    ...
    return(np.row_stack((FirstbetaR1_C,FirstbetaR2_C,FirstbetaT_C,Firstu1_C,Firstu2_C,Firstv)))

def Hessian_C(betaR1,betaR2,betaT,u1,u2,v,theta1,theta2,theta3,ro1,ro2,ro3,TMB_C,X_E,z,deltaT,R1,R2,M):
    sc1 = 0
    sc2 = 0
    sc3 = 0
    sc4 = 0
    ...
    for i in range(K):
        ...
        W1 = (np.exp(yetaR1_C)+np.exp(yetaR1_C+yetaR2_C)) / ((1+np.exp(yetaR1_C)+np.exp(yetaR2_C))**2)
        SecdR11_C = np.diag(W1.reshape(-1))
        W2 = (np.exp(yetaR2_C)+np.exp(yetaR1_C+yetaR2_C)) / ((1+np.exp(yetaR1_C)+np.exp(yetaR2_C))**2)
        SecdR22_C = np.diag(W2.reshape(-1))
        W3 = -np.exp(yetaR1_C+yetaR2_C) / ((1 + np.exp(yetaR1_C) + np.exp(yetaR2_C))**2)
        SecdR12_C = np.diag(W3.reshape(-1))
        
        sc1 += (np.dot(np.dot(X_C.T, SecdR11_C), X_C)).real
        sc2 += (np.dot(np.dot(X_C.T, SecdR12_C), X_C)).real
        sc3 += (np.dot(np.dot(X_C.T, SecdR11_C), z)).real
        sc4 += (np.dot(np.dot(X_C.T, SecdR12_C), z)).real
        sc5 += (np.dot(np.dot(X_C.T, SecdR22_C), X_C)).real
        sc6 += (np.dot(np.dot(X_C.T, SecdR22_C), z)).real       
        sc7 += (np.dot(np.dot(z.T, SecdR11_C), z)).real
        sc8 += (np.dot(np.dot(z.T, SecdR12_C), z)).real
        sc9 += (np.dot(np.dot(z.T, SecdR22_C), z)).real   
  
    yetaT = np.dot(X_E, betaT) + np.dot(z,v)   	    
    W4 = np.exp(yetaT)
    A = deltaT / np.dot(M.T, (np.exp(yetaT)))
    B = np.dot(M, A)
    SecdT = np.diag((W4*B).reshape(-1))-np.dot(np.dot(np.dot(np.dot(np.diag(W4.reshape(-1)),M),
                                                             np.diag((A*A).reshape(-1))), M.T), 
                                               np.diag(W4.reshape(-1)))
    
    SecdbetaT_11_C = np.sum(deltaT*((np.dot(M.T, (X_E[:,0:1]**2)*np.exp(yetaT))*np.dot(M.T, np.exp(yetaT))
                           -np.dot(M.T, X_E[:,0:1]*np.exp(yetaT))**2)
                          /(np.dot(M.T, np.exp(yetaT))**2)))
    SecdbetaT_22_C = np.sum(deltaT*((np.dot(M.T, (X_E[:,1:2]**2)*np.exp(yetaT))*np.dot(M.T, np.exp(yetaT))
                           -np.dot(M.T, X_E[:,1:2]*np.exp(yetaT))**2)
                          /(np.dot(M.T, np.exp(yetaT))**2)-sigma_err**2))
    SecdbetaT_12_C = np.sum(deltaT*((np.dot(M.T, X_E[:,0:1]*X_E[:,1:2]*np.exp(yetaT))*np.dot(M.T, np.exp(yetaT))
                           -np.dot(M.T, X_E[:,0:1]*np.exp(yetaT))*np.dot(M.T, X_E[:,1:2]*np.exp(yetaT)))
                          /(np.dot(M.T, np.exp(yetaT))**2)))   
    #SecdbetaT_1v_C = np.sum(deltaT*(((X_E[:,0:1]**2)*np.exp(yetaT)*np.dot(M.T, np.exp(yetaT))
    #                       -np.dot(M.T, X_E[:,0:1]*np.exp(yetaT))*np.exp(yetaT))
    #                      /(np.dot(M.T, np.exp(yetaT))**2)))
    Info11 = sc1/K
    Info12 = sc2/K
    ...
    Info31 = Info13.T
    Info32 = Info23.T
    Info33 = np.array(([SecdbetaT_11_C,SecdbetaT_12_C],[SecdbetaT_12_C,SecdbetaT_22_C]))#np.dot(np.dot(X_E.T, SecdT), X_E)   
    Info34 = np.zeros((num,n))
    Info35 = np.zeros((num,n))
    Info36 = np.dot(np.dot(X_E.T, SecdT), z)
      
    Info41 = Info14.T
    Info42 = Info24.T
    Info43 = Info34.T
    Info44 = sc7/K + ((1-ro3**2)/(theta1*(1-(ro1**2+ro2**2+ro3**2-2*ro1*ro2*ro3))))*np.identity(n)
    Info45 = sc8/K + ((ro2*ro3-ro1)/(math.sqrt(theta1*theta2)*(1-(ro1**2+ro2**2+ro3**2-2*ro1*ro2*ro3))))*np.identity(n)
    ...
    Info53 = Info35.T
    Info54 = Info45.T
    Info55 = sc9/K + ((1-ro2**2)/(theta2*(1-(ro1**2+ro2**2+ro3**2-2*ro1*ro2*ro3))))*np.identity(n)
...
def BLUP_VAR(T44,theta1,theta2,theta3,ro1,ro2,ro3):
    ...
    #Omega = np.row_stack((np.column_stack((theta1*np.identity(n), ro1*math.sqrt(theta1*theta2)*np.identity(n), ro2*math.sqrt(theta1*theta3)*np.identity(n))),
    #              np.column_stack((ro1*math.sqrt(theta1*theta2)*np.identity(n), theta2*np.identity(n), ro3*math.sqrt(theta2*theta3)*np.identity(n))),
    #              np.column_stack((ro2*math.sqrt(theta1*theta3)*np.identity(n), ro3*math.sqrt(theta2*theta3)*np.identity(n), theta3*np.identity(n)))))
    A1 = theta1*np.identity(n)
    A2 = ro1*math.sqrt(theta1*theta2)*np.identity(n)
    A3 = ro2*math.sqrt(theta1*theta3)*np.identity(n)   
    #A4 = ro1*math.sqrt(theta1*theta2)*np.identity(n)    
    A5 = theta2*np.identity(n)    
    A6 = ro3*math.sqrt(theta2*theta3)*np.identity(n)    
    #A7 = ro2*math.sqrt(theta1*theta3)*np.identity(n)
    #A8 = ro3*math.sqrt(theta2*theta3)*np.identity(n)   
...
for iter0 in range(itmax):
    for iter1 in range(itmax):
        ...
        para1 = para + np.dot(Hessian_C(betaR1,betaR2,betaT,u1,u2,v,theta1,theta2,theta3,ro1,ro2,ro3,TMB_C,X_E,z,delta,R1,R2,M),
                              Score_C(betaR1,betaR2,betaT,u1,u2,v,theta1,theta2,theta3,ro1,ro2,ro3,TMB_C,X_E,z,delta,R1,R2,M))	        	
        betaR1 = para1[0:num]
    ...
    Info = Hessian_C(betaR1,betaR2,betaT,u1,u2,v,theta1,theta2,theta3,ro1,ro2,ro3,TMB_C,X_E,z,delta,R1,R2,M)
